...
public final class fragRobotEffects extends lpBaseFragment<lpChangeEffectViewModel, FragAllEffectsBinding> {
    public lpItemEffectAdapterLp lpItemEffectAdapter;
    private final BroadcastReceiver lpreceiver = new BroadcastReceiver() { // from class: com.app.voicechangereffect.fragments.fragRobotEffects.1
        @Override // android.content.BroadcastReceiver
        public void onReceive(Context context, Intent intent) {
            lpEffectModel lpeffectmodel;
            lpItemEffectAdapterLp lpitemeffectadapterlp;
            if (!Intrinsics.areEqual(intent == null ? null : intent.getAction(), "select_effect") || (lpeffectmodel = (lpEffectModel) intent.getParcelableExtra("effect_model")) == null || (lpitemeffectadapterlp = fragRobotEffects.this.lpItemEffectAdapter) == null) {
                return;
            }
    ...
    public void bindViews() {
        ...
        List<lpEffectModel> allRobotEffects = getLpviewModel().getAllRobotEffects(requireContext());
        getLpdataBinding().lprclEffect.setHasFixedSize(true);

        lpItemEffectAdapterLp lpitemeffectadapterlp = new lpItemEffectAdapterLp(requireContext(), allRobotEffects, it -> {
            Intrinsics.checkNotNullParameter(it, "it");
            lpItemEffectAdapterLp lpitemeffectadapterlp2 = fragRobotEffects.this.lpItemEffectAdapter;
            if (lpitemeffectadapterlp2 != null) {
                lpitemeffectadapterlp2.lpselectEffectItem(it);
            }
            ((lpChangeEffectActivity) fragRobotEffects.this.requireContext()).lpplayEffect(it.getLpid());
